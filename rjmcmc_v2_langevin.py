...
class MCMC:
    def __init__(self, use_langevin_gradients, langevin_prob, samples, traindata, testdata, minPerf, mtaskNet):
    ...
    def sampler(self, dims1, dims2, w_limit, eta_limit):
        ...
        for i in range(samples -1):
            if is_in_lower_dimension: #jump up dimension: netw1 < netw2
                ...
                log_posterior_down = (prior_current + likelihood_current) - (prior_proposal + likelihood_proposal + log_v) - diff_prop 
                try:
...
def main():
    for problem in range(2, 3): 
        ...
        mcmc = MCMC(use_langevin_gradients, langevin_prob, numSamples, traindata, testdata, minPerf, mtaskNet)
